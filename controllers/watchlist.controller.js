...
const watchlistController = {
    ...
    getListCourses: async(req, res) => {
        if (req.session.isAuth === true) {
            ...
            const courseInCat = await watchlistModel.allCatWithDetails(req.session.authUser.userID);
            //console.log(courseInCat);
            // const allCourse = await courseModel.all();
            const allCourse = await watchlistModel.allCourse(req.session.authUser.userID);
            ...
            const rows = await watchlistModel.pagiListCourse(offset, limit, req.session.authUser.userID);
    ...
    getCourseListByCat: async(req, res) => {
        if (req.session.isAuth === true) {
            ...
            const rows = await watchlistModel.pagiListCourseByCat(catID, offset, limit, req.session.authUser.userID);
